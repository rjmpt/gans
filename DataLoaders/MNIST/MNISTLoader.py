...
class DataLoader:
    ...
    def load_h5(self):
        processed_file_path = self.dataset_dir + 'mnist.h5'
    ...
    def create_h5(self):
        ...
        training_images = (mnist.train.images*255.).astype(np.uint8).reshape(self.image_size)
        validation_images = (mnist.validation.images*255.).astype(np.uint8).reshape(self.image_size)
        test_images = (mnist.test.images*255.).astype(np.uint8).reshape(self.image_size)
